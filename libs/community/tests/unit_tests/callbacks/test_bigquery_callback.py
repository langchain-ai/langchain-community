...
def mock_bigquery_clients() -> Generator[Dict[str, Any], None, None]:
    ...
    mock_sync_write_client_cls.BigQueryWriteClient.return_value = (
    ...
    with patch(
        "langchain_community.callbacks.bigquery_callback.guard_import",
        side_effect=guard_import_side_effect,
    ), patch(
        "sys.modules", {"pyarrow": mock_pyarrow, "google.api_core": mock_api_core, **sys.modules}
    ):
...
def test_sync_ensure_init_creates_dataset_and_table(
    ...
    mock_bq_client.get_table.side_effect = mock_cloud_exceptions.NotFound("Table not found")
    handler._ensure_started()
