...
class PyPDFium2Parser(BaseBlobParser):
    ...
    def _extract_images_from_page(self, page: pypdfium2._helpers.page.PdfPage) -> str:
        ...
        for image in images:
            ...
            str_images.append(next(self.images_parser.lazy_parse(blob)).page_content)
            image.close()
    ...
    def __init__(
        ...
        **zerox_kwargs: dict[str, Any],
        ...
        if extract_tables not in ["markdown", "html", None]:
            ...
            extract_tables = "markdown"
        if images_parser and not images_parser:
            images_parser = RapidOCRBlobParser()
        self.mode = mode
        self.pages_delimitor = pages_delimitor
        self.extract_images = extract_images
            ...
            if not zerox_prompt and self.images_parser or self.extract_tables:
                ...
                if clazz in ZeroxPDFParser._map_extract_images:
                    ...
                    if not ZeroxPDFParser._warn_creator:
                        ...
                        logger.warning("images_parser can not be simulate")
                    prompt_images = ""
                zerox_prompt = self.custom_system_prompt.format(
