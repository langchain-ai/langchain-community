        ...
        for row in rows:
            for field in self.arrow_schema:
                ...
                if field_metadata and b"ARROW:extension:name" in field_metadata:
                    if (
                        field_metadata[b"ARROW:extension:name"]
    ...
    async def _write_rows_with_retry(self, rows: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            req = self.bq_storage_types.AppendRowsRequest(
                write_stream=self.write_stream, trace_id=f"langchain-bq-callback"
            )
        ...
        while attempt <= self.retry_config.max_retries:
            try:
                ...
                logger.warning(
                    "BigQuery write failed (Attempt %s), retrying in %.2fs... Error: %s",
                    attempt,
                    sleep_time,
                    e,
                )
...
class _SyncBatchProcessor(_BatchProcessor):
    ...
    def __init__(
        ...
        # We don't call super().__init__ because it initializes async-specific attributes.
        # Instead, we initialize the required attributes for the sync version directly.
        self.write_client = write_client
        self.arrow_schema = arrow_schema
        self.write_stream = write_stream
        self.batch_size = batch_size
    ...
    async def _ensure_started(self) -> None:
        ...
        async with self._setup_lock:
            ...
            client_info = self.gapic_client_info.ClientInfo(
                user_agent=f"langchain-bq-callback"
            )
    ...
    async def on_llm_start(
        ...
        **kwargs: Any,
        ...
        await self._log(
            ...
            attributes={"tags": tags, "model": serialized.get("name")},
...
class BigQueryCallbackHandler(BaseCallbackHandler):
    ...
    def _get_events_schema(self) -> list[Any]:
        return AsyncBigQueryCallbackHandler._get_events_schema(self)

    def to_arrow_schema(self, bq_schema_list: list[Any]) -> Optional[Any]:
        return AsyncBigQueryCallbackHandler.to_arrow_schema(self, bq_schema_list)
