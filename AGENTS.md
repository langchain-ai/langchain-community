...
This repo contains LangChain community integrations, as published by `langchain-community` in `libs/community`. It strongly depends on the `langchain-ai/langchain` monorepo, which contains the core LangChain functionality. This document provides context to understand the LangChain Python project and assist with development.

If needed, the langchain monorepo is usually cloned at the same level as this repo, so you can refer to its code directly by navigating to `../langchain/` from this repo.

### `langchain-ai/langchain` Monorepo structure
